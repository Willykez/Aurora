package com.auroraai.chat.ui.components
...
import com.auroraai.chat.ui.markdown.MarkdownText
import com.auroraai.chat.ui.theme.ErrorRed
...
fun ChatBubble(
    ...
    if (role == "user") {
        ...
        AssistantTurn(text, isStreaming, isError, modifier)
    }
}
...
@Composable
private fun AssistantTurn(text: String, isStreaming: Boolean, isError: Boolean, modifier: Modifier) {
    val clipboard = LocalClipboardManager.current
    Column(modifier.fillMaxWidth().padding(horizontal = 2.dp)) {
        if (text.isNotBlank()) {
            MarkdownText(text, isStreaming = isStreaming)
        } else if (isStreaming) {
            // Nothing on screen yet — rotating phrases instead of a single static "Thinking…"
            // that never changes and reads as stuck after a few seconds.
            ThinkingIndicator()
        }
        if (isError) {
