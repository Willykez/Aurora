...
class AiClient(private val config: ProviderConfig) {
     ...
     * close_notify) on whatever thread calls it. The caller — the `finally` block after a
     * streamed reply — runs on viewModelScope's Main dispatcher, so evicting synchronously
     * there is a guaranteed NetworkOnMainThreadException (Android enforces this at the OS level
     * for targetSdk ≥ 11; it's not just a StrictMode warning). Running the actual eviction on a
     * throwaway thread makes close() safe to call from anywhere, including inside a cancelled
     * coroutine's `finally` block, without the caller needing to remember withContext(Dispatchers.IO).
     */
    fun close() {
        Thread {
            runCatching {
